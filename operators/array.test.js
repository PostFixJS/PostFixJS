...
test('path-update should set a value in a one-dimensional array', (t) => {
  const { stack } = execute('[:a 1 :b 3] [:b] 0 { 1 - } path-update')
  t.is(stack.count, 1)
  t.is(stack.pop().items[3].value, 2)
})

test('path-update should set a value in a missing sub-array', (t) => {
  const { stack } = execute('[] [:x :y :z] 21 { 2 * } path-update')
  t.is(stack.count, 1)
