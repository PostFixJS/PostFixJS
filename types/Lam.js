...
class Lam extends ExeArr {
  ...
  * execute (interpreter, { callerToken } = {}) {
    if (this.dict) {
      // usually, lambda functions have dicts (those defined in PostFix always do)
      // those generated by datadef, however, don't because sometype-do could not use the current
      // dictionary then, leading to errors that a variable can't be found although it is in the
      // dictionary outside of the lambda function (the user doesn't see the generated code)
  ...
  _copyImpl () {
    return new Lam(
      [...this.items],
      this.params.copy(),
      this.dict ? Object.assign({}, this.dict) : null
    )
  }
}
